...
export function DeckosAudio({ track, state, elapsed, volume, shuffle, repeat, chrome, onToggle, onNext, onPrev, onToggleShuffle, onToggleRepeat, onSeek }: DeckosAudioProps & { chrome?: ChromeApi; onSeek?: (sec: number) => void }) {
  ...
  return (
    <DeckChrome title="DECK//OS" chrome={chrome}>
      <div style={{padding: "24px 28px", display: "grid", gridTemplateColumns: "minmax(0, 1fr) 360px", gap: 28, height: "calc(100% - 96px)"}}>
        {/* Left: track + spectrum. minWidth:0 + minmax(0,1fr) on the
         * track template lets the grid track shrink below its content's
         * intrinsic width — otherwise the AsciiBar (70 chars) inside the
         * progress bar expands this column past 836 px and the right
         * column drifts off the 1280-design-wide skin. */}
